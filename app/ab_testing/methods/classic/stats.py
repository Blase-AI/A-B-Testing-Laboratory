...
def test_normality(data_a: np.ndarray, data_b: np.ndarray) -> dict[str, dict[str, Any]]:
    ...
    p_a = float(shapiro_a.pvalue)
    p_b = float(shapiro_b.pvalue)
    return {
        "Group A": {"p_value": p_a, "normal": bool(p_a > 0.05)},
        "Group B": {"p_value": p_b, "normal": bool(p_b > 0.05)},
    }
...
def perform_t_or_u_test(
    data_a: np.ndarray, data_b: np.ndarray, *, alpha: float = 0.05
) -> dict[str, Any]:
    ...
    if norm_results["Group A"]["normal"] and norm_results["Group B"]["normal"]:
        t_stat, p_value = stats.ttest_ind(
            data_a, data_b, equal_var=variance_results["equal_variance"]
        )
        p = float(p_value)
        result = {"test": "t-test", "t_stat": float(t_stat), "p_value": p}
    else:
        u_stat, p_value = stats.mannwhitneyu(data_a, data_b, alternative="two-sided")
        p = float(p_value)
        result = {"test": "Mann-Whitney U-test", "u_stat": float(u_stat), "p_value": p}

    result["alpha"] = float(alpha)
    result["significant"] = bool(p < float(alpha))
    return result
...
def cohens_d(data_a: np.ndarray, data_b: np.ndarray) -> float:
    ...
    pooled_std = np.sqrt(
        ((nx - 1) * np.std(data_a, ddof=1) ** 2 + (ny - 1) * np.std(data_b, ddof=1) ** 2)
        / (nx + ny - 2)
    )
...
def mean_difference_ci(
    data_a: np.ndarray, data_b: np.ndarray, *, confidence: float = 0.95
