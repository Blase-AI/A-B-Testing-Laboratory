...
def test_sequential_sprt_returns_result_and_plots():
    ...
    f0 = lambda x: norm.pdf(x, loc=0.0, scale=1.0)
    f1 = lambda x: norm.pdf(x, loc=0.3, scale=1.0)
...
def test_normal_sprt_simulator_smoke():
    ...
    sim = NormalSPRTSimulator(mu0=0.0, mu1=0.3, sigma=1.0, n=50, alpha=0.05, true_state="H1", stop_threshold=30)
    res = sim.run()
